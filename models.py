...
class Abstract_Model(object):
	...
	def get_loss_args_and_symb_vars(self, train_triples, valid_triples, hparams):
		...
		train = batching.Batch_Loader(train_triples, n_entities = max(self.n,self.l), batch_size = hparams.batch_size, neg_ratio = hparams.neg_ratio, contiguous_sampling = hparams.contiguous_sampling ) 
		inputs=[self.ys, self.rows, self.cols, self.tubes]
		if valid_triples is not None:
			valid = batching.Batch_Loader(valid_triples, n_entities = max(self.n,self.l), batch_size = hparams.batch_size, neg_ratio = hparams.neg_ratio, contiguous_sampling = hparams.contiguous_sampling ) 
				...
				if valid_triples is not None:
					...
					if scorer.compute_ranking_scores:
						...
						if best_valid_mrr >= metrics[self.name][2]:
							tools.logger.info("Validation filtered MRR decreased, stopping here.")
							break
						else:
							best_valid_mrr = metrics[self.name][2]
					else:
						tools.logger.info("Validation AP: " + str(res.ap))
						#Early stopping on Average Precision
						if best_valid_ap >= res.ap:
							tools.logger.info("Validation AP decreased, stopping here.")
							break
						else:
...
class Complex_Model(Abstract_Model):
	...
	def get_init_params(self):
		...
		params = { 'e1' : tools.randn(max(self.n,self.l),self.k),
				   'e2' : tools.randn(max(self.n,self.l),self.k),
				   'r1' : tools.randn(self.m,self.k),
				   'r2' : tools.randn(self.m,self.k)  }
		return params
...
class Rescal_Model(Abstract_Model):
	...
	def get_init_params(self):
		...
		params = { 'r' : tools.randn(self.m, self.k, self.k),
				   'e' : tools.randn(max(self.n,self.l),self.k)  }
		return params
